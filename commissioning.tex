...
The first Rubin on-sky commissioning campaign was conducted using the \gls{LSSTComCam}. 
The campaign's primary objective was to optically align the Simonyi Survey Telescope and verify its ability to deliver acceptable image quality using \gls{LSSTComCam}.
...
Approximately 16,000 exposures were collected during this campaign across all six filters, spanning a wide range of observing conditions, airmass, and stellar densities. 
The majority were obtained in support of \gls{AOS} commissioning, system-level verification, and end-to-end testing of the telescope’s hardware and software. 
...
For \gls{DP1}, we selected a subset of \nexposures{} science-grade exposures from this campaign, covering seven non-contiguous fields. 
These fields were deliberately chosen to sample a range of stellar densities, overlap with external reference datasets, and span the full breadth of the four primary LSST science themes in order to enable the maximum of early science 
...
It is designated as Raft 22 (R22).
\figref{fig:comcam_focal_plane} shows the LSSTComCam focal plane layout illustrating the placement and numbering scheme of sensors (S) and amplifiers (C) within  Raft 22 (R22).
...
Some sensors exhibit known issues such as elevated readout noise (e.g., Detector 8, S22) and increased \gls{CTI} (e.g., Detector 5, S12).
\begin{figure}[htb!]
...
The corresponding transmission curves are provided as a DP1 data product and are described in \secref{sssec:transmission_curves}
...
Future campaigns with LSSTCam will benefit from more stable and uniform flat fielding using the Rubin flat field system, described in \citet{SITCOMTN-086}.
...
\figref{fig:dp1_fields_on_sky} shows the locations of these \nfields fields on the sky, overlaid on the LSST baseline survey footprint \citep{PSTN-051, PSTN-052, PSTN-053, PSTN-055, PSTN-056}, as well as the area of sky covered by both the \gls{LSSTCam} and \gls{LSSTComCam} focal planes.
\begin{figure*}[htb!]
\centering
...
The fields of view covered by the \gls{LSSTCam} and \gls{LSSTComCam} focal planes are represented as outer and inner concentric circles, respectively, centered on the pointing center of each field.}
...
Additionally, some $u$-band exposures were taken as 1$\times$38-second exposures.
...
It is important to note that the time sampling in the \gls{DP1} dataset differs significantly from that of  \gls{LSST} survey cadence.
\begin{figure}[htb!]
\includegraphics[width=\linewidth]{visitDates}
\caption{Distribution of DP1 observations by date grouped by field as a function of time over the \nnightscomcam nights of data taking with LSSTComCam.Each dot represents a single  exposure, color-coded by band.}
...
\figref{fig:dp1_fields_coverage} shows sky coverage maps showing the distribution of visits in each field, color coded by band. 
...
The best images achieve a \gls{PSF} \gls{FWHM}  of approximately \bestimagequality, with  per-band median  values of: u = \umedianimagequality,  g = \gmedianimagequality, r = \rmedianimagequality, i = \imedianimagequality, z = \zmedianimagequality, y = \ymedianimagequality arcsecs.
Both the per-sensor \gls{PSF} \gls{FWHM} and the overall median vary by filter and  specific target field. 
