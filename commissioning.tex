...
While the \gls{LSSTComCam} cryostat employs a different cooling system (Cryotels) to that of \gls{LSSTCam}, it included a refrigeration pathfinder to validate the cryogenic system intended for \gls{LSSTCam}.
...
\figref{fig:lsstcam_focal_plane_with_comcam} presents a schematic of the \gls{LSSTCam} focal plane, with the \gls{LSSTComCam} raft positioned at the center, corresponding to the \gls{LSSTCam} central science raft location.
...
\caption{Schematic showing the single LSSTComCam science raft positioned at the center of the full LSSTCam focal plane. The perspective is from above, looking down through the LSSTComCam lenses onto the focal plane. Credit: RubinObs/NOIRLab/SLAC/NSF/DOE/AURA.}
\label{fig:lsstcam_focal_plane_with_comcam}
\end{figure} 
...
This design is identical across all science rafts, and to maintain uniform performance and \gls{calibration}, each raft is populated exclusively with sensors from a single vendor.
...
The absolute time accuracy of data taken with LSSTComCam relies on the Network Time Protocol (NTP) for clock synchronization, which should be accurate to approximately 1 millisecond. 
...
EUTELSAT 117 West B is part the GPS system and serves as one of WAAS (Wide Area Augmentation System) satellites operated for the U.S. Federal Aviation Administration (FAA) and used to broadcast GPS corrections to air traffic.
...
As the LSSTComCam sensors are centered in the aperture, the center of the focal plane should be exposed about half-way through the shutter open procedure, 198 milliseconds after the open command. 
...
The DP1 median delivered image quality, quantified as the \gls{PSF} at \gls{FWHM} across all filters and target fields, is \medianimagequalityallbands. 
...
\begin{figure*}[hbt!]
  ...
  \begin{subfigure}[t]{0.45\textwidth}
    ...
    \caption{PSF FHWM (arcsecs) per passband across all DP1 target fields. } 
 \end{subfigure}\hfill
  \begin{subfigure}[t]{0.45\textwidth}
    \includegraphics[width=\linewidth]{image_quality_histo_per_field}
    \caption{PSF FHWM (arcsecs) per DP1 target field across all passbands}
  \end{subfigure}\hfill
\caption{Histograms showing the  distribution of delivered image quality for all \nvisitdetectorsummaries single-epoch individual sensors in the DP1 dataset per passband (a) and per field (b). The median values are given in the legend. }
