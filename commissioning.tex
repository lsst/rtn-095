...
Each night began with an initial alignment correction using a laser tracker to position the system within the capture range of the closed-loop algorithm \citep{10.1117/12.3019031}. 
...
During Science Pipelines commissioning (\secref{ssec:pipelines_commissioning}), observations were undertaken using the open-loop component with no correction for thermal effects. 
...
LSSTComCam consists of a single raft equipped exclusively with ITL sensors.
...
\figref{fig:comcam_raft_in_lsstcam_focal_plane} shows the single-raft LSSTComCam positioned at the center of the full LSSTCam focal plane. 
...
\figref{fig:comcam_focal_plane} shows the LSSTComCam focal plane layout, illustrating the enumeration of sensors and amplifiers, along with their physical arrangement within the raft.
\begin{figure}[htb!]
\centering
...
The full-system throughput of the six LSSTComCam filters, which encompasses contributions from a standard atmosphere at airmass 1.2, telescope optics, camera surfaces, and the mean ITL detector quantum efficiency is shown in \figref{fig:comcam_standard_bandpasses}.
\begin{figure}[htb!]
\centering
...
Gaps in coverage across some bands arise from the fact that LSSTComCam can only accommodate three filters at a time \secref{ssec:comcam}. 
...
\begin{figure*}[ht]
    ...
    \caption{Sky coverage for seven DP1 fields.}
