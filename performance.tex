...
Applying a reliability threshold based on the Machine Learning reliability model described in \autoref{ssec:diffim_analysis}
improved the purity of transient detections but had limited impact on variable stars. 
...
The performance of the reliability model on the test data (see \autoref{ssec:diffim_analysis}) is shown in \aturef{completenessvpurity}.
The rate of true positives and false negatives obtained by thresholding the reliability score at 0.5 is reported for transients (99 stamps), and variable stars (316 stamps) vetted in TASSO in \autoref{tab:mlreliability}. Additionally we crossmatched stamps with Solar System Objects with known orbits retriving 5,988 Solar System Objects stamps.

\figref{fig:mlreliability_pc} Purity and Completness as a function of machine learning reliability threshold for test data.
\begin{figure}[htb!]
\includegraphics[width=\linewidth]{mlreliability_test_performance.pdf}
\caption{The Purity and Completness of the reliability score is shown as a function of reliability threshold based on the testing data. A vertical line marks the threshold where the highest F1-score is obtained. The F1 score is the harmonic mean of completeness and purity.
  See \aturef{ssec:diffim_analysis} for details on the model and model training.}
\label{fig:completenessvpurity}
\end{figure}

\setlength{\tabcolsep}{6pt}  % spacing between columns
\begin{deluxetable}{lccc}
  \tablecaption{Recovery rate for transients, variables, and Solar System objects with a reliability threshold set to 0.5.
\label{tab:mlreliability}}
\tablehead{
  \colhead{\textbf{Object type}} & \colhead{\textbf{Number of objects}} & \colhead{\textbf{True Positive Rate}} & \colhead{\textbf{False Negative Rate}} 
}
\startdata
Solar System Objects & \parbox{0.3cm}{5,988} & \parbox{0.3cm}{93.5\%} & \parbox{0.3cm}{6.5\%}\\
Transients & \parbox{0.3cm}{99} & \parbox{0.3cm}{73.7\%} & \parbox{0.3cm}{26.3\%}\\
Variables & \parbox{0.3cm}{316} & \parbox{0.3cm}{3.5\%} & \parbox{0.3cm}{96.5\%}\\
\enddata
\end{deluxetable}
