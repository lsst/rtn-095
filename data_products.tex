...
Here we describe Rubin \gls{DP1} data products and provide summary statistics for each, but we also refer the reader to the DOI-registered \gls{DP1} release documentation available at \url{https://dp1.lsst.io} and the catalog schemas available at \url{https://sdm-schemas.lsst.io}.\footnote{Searchable catalog schemas are also available to Data Rights Holders via the Rubin Science Platform at \url{https://data.lsst.cloud}.}
...
To aid legibility, we have separated the descriptions of the data products from the description of the data release processing pipeline, a summary of which is provided in \secref{sec:drp}.
Similarly, because most of the data products described here can be accessed via either the \gls{IVOA} Services ( \secref{sssec:ivoa_services}) or the Data \gls{Butler} (\secref{sssec:data_butler}), we describe them in a manner that is agnostic to the means of access.
...
Flux measurements are more appropriate for multi-epoch observations, as fluxes should be averaged across epochs rather than magnitudes.
Additionally, flux measurements on difference images (\secref{ssec:science_images}) are performed against a template, representing a flux difference. As a result, the measured flux of a source on a difference image can be negative, particularly for faint sources in the presence of noise.
...
Fluxes for various apertures are provided and named in the format \texttt{[band]\_ap[size]Flux}, where \texttt{[size]} represents the aperture diameter in pixels. For example, \texttt{g\_ap03Flux} refers to the flux measured within a 3.0-pixel aperture in the g-band.
...
A full set of columns  with description and units in all table in DP1 can be found at \nolinkurl{https://sdm-schemas.lsst.io/dp1.html}
Where applicable, all measured properties are reported with their associated 1$\sigma$ uncertainties.

Since \gls{DP1} is a preview, it does not include all the catalogs expected in a full \gls{LSST} \gls{Data Release}. Additionally, the catalogs it does include may be missing some columns planned for future releases.
Where this is known to be the case, we note what data are missing in the catalog descriptions that follow.
